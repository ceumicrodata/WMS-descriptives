...
@generate round_age = (trunc(Union{Int64, Missing}, firm_age / 10)*10 == firm_age)
# if old age is not rounded, it is likely a foundation year
...
@generate round_age = (trunc(Union{Int64, Missing}, plant_age / 10)*10 == plant_age)
# if old age is not rounded, it is likely a foundation year
...
@generate round_age = (trunc(Union{Int64, Missing}, comp_tenure / 10)*10 == comp_tenure)
# if old age is not rounded, it is likely a foundation year
...
@generate round_age = (trunc(Union{Int64, Missing}, post_tenure / 10)*10 == post_tenure)
# if old age is not rounded, it is likely a foundation year
@replace post_tenure = 2018 - (1900+post_tenure) @if (post_tenure > 60) & !ismissing(post_tenure) & !round_age
@drop round_age

@generate production_list = [["termel", "gyart", "gyárt", "uzem", "üzem", "oper", "techn", "műszaki", "projekt", "telephely"]]
@generate ceo_list = [["ügyvezet", "vezér", "elnük", "vezer", "cégvez", "tulaj", "ugyvez"]]

@replace position = lowercase(position)
@generate pos = ""
# Okay I think this forloop can be avoided....
@replace pos = "production" @if any(production_list in position)
@replace pos = "ceo" @if any(ceo_list in position)
@replace pos = "other" @if ismissing(pos)
@drop production_list ceo_list
...
any_cols = [col for col in names(getdf()) if eltype(getdf()[!,col]) == Any]
df = getdf()
for col in any_cols
    df[!, col] = convert(Vector{Union{AbstractString, Missing}},df[!, col])
end
setdf(df)
